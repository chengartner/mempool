...
// Matmul based on outer product between rows of A and cols of B
  // Use 4 cols of A (store 4 elements each) and 4 rows of B (store 4 elements each)
  // 2D parallelize the workload into 8x4 submatrices of matrix B
void matmul_4x4_2d_parallel_outer_f8vec(const __fp8 *__restrict__ A,
                                const __fp8 *__restrict__ B,
                                __fp8 *__restrict__ C, uint32_t M,
                                uint32_t N, uint32_t P, uint32_t core_id,
                                uint32_t numThreads) {
  ...
  // Parallelize by assigning each core 8 rows of matrix B
  uint32_t const r = N / 8; // How many rows to split the matrix into
  uint32_t const row_start = 8 * (core_id % r);
  uint32_t const row_end = 8 * ((core_id % r) + 1);

  // Parallelize by assigning each core 4 cols of matrix B
  uint32_t const c = P / 4; // How many cols to split the matrix into
  uint32_t const col_start = 4 * (core_id % c);
  uint32_t const col_end = 4 * ((core_id % c) + 1);

  for (k = col_start; k < col_end; k += 4) {
    for (i = 0; i < M; i += 4) {
      v4b sum0 = (v4b)0.0f;
      v4b sum1 = (v4b)0.0f;
      v4b sum2 = (v4b)0.0f;
...
// Matmul based on inner product between rows of A and cols of B
  // Use 4 rows of A (store 4 elements each) and 4 cols of B (store 4 elements each)
  // 2D parallelize the workload into 8x4 submatrices of matrix B
void matmul_4x4_2d_parallel_inner_f8vec(const __fp8 *__restrict__ A,
                                const __fp8 *__restrict__ B,
                                __fp8 *__restrict__ C, uint32_t M,
                                uint32_t N, uint32_t P, uint32_t core_id,
                                uint32_t numThreads) {
