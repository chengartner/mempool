  ...
  for (i = core_id * 2; i < M; i += numThreads * 2) {
    ...
    for (j = 0; j < N; j += 4) {
      ...
      asm volatile(
        ...
        "vfmul.h %[aVec00], %[aVec00], %[aVec00];"  // aVec00 stores square of loaded aVec00
        "vfmul.h %[aVec10], %[aVec10], %[aVec10];"  // aVec10 stores square of loaded aVec10
        ...
        : [vSum0] "+&r"(vSumMean0), [vSum1] "+&r"(vSumMean1), [vSumSq0] "+&r"(vSumSq0),
          [vSumSq1] "+&r"(vSumSq1), [aVecSq01] "+&r"(aVecSq01),
          [aVecSq11] "+&r"(aVecSq11)
    ...
    for (j = 0; j < N; j += 4) {
      ...
      asm volatile(
        ...
        : [vMean0] "r"(vSumMean0), [vMean1] "r"(vSumMean1), [vStd0] "r"(vSumSq0),
          [vStd1] "r"(vSumSq1));
