...
void softmax_parallel_2x8_f8vec(const __fp8 *__restrict__ A,
  ...
  // 1) Find the row-wise maximum value
    ...
    asm volatile(
      ...
      "vfmax.b %[max0], %[x0], %[x1];"
      "vfmax.b %[max1], %[x2], %[x3];"
      : [max0] "+&r"(max0), [max1] "+&r"(max1), [x0] "+&r"(x0), [x1] "+&r"(x1), 
        [x2] "+&r"(x2), [x3] "+&r"(x3), [pmax10] "+&r"(pmax10), 
        [pmax11] "+&r"(pmax11), [pmax12] "+&r"(pmax12), [pmax13] "+&r"(pmax13)
  ...
  // 2) Compute exp(x - max)
    ...
    for (j = 0; j < N; j += 8) {
      ...
      asm volatile(
        ...
        "vfadd.b %[xSq00], %[xSq00], %[xCu00];"   // 1 + 1*x + 0.5*x^2 + 0.167*x^3
      ...
      asm volatile(
        "vfadd.b %[sum0], %[exp00], %[exp01];"  // Use aVec00 as temporary sum variable
    ...
    asm volatile(
      ...
      : [sum02] "+&r"(sum02), [sum03] "+&r"(sum03), 
        [vSum0] "+&r"(x0), [vSum1] "+&r"(x1), [sum00] "+&r"(x2), [sum01] "+&r"(x3),
