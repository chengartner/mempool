  ...
  // 1) Find the row-wise maximum value
    ...
    asm volatile(
      ...
      : [max0] "+&r"(max0), [max1] "+&r"(max1),
        [pmax00] "+&r"(x0), [pmax01] "+&r"(x1),
        [pmax10] "+&r"(x2), [pmax11] "+&r"(x3)
  ...
  // 2) Compute exp(x - max)
    ...
    for (j = 0; j < N; j += 4) {
      ...
      asm volatile(
        ...
        "vfadd.h %[xSq00], %[xSq00], %[xCu00];"   // 1 + 1*x + 0.5*x^2 + 0.167*x^3
      ...
      asm volatile(
        "vfadd.h %[sum0], %[exp00], %[exp01];"  // Use aVec00 as temporary sum variable
